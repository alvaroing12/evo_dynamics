...
\begin{align*}
    ...
    \mathbf{x}^{*} (W - \phi I) &= 0
\end{align*}
This holds iff \( \phi \) is an eigenvalue of \( W \) and \( \mathbf{x}^{*} \) is the corresponding left-eigenvector. Thus \( \phi \in \{\lambda_1, \lambda_2\} \). We know that \( \lambda_{1, 2} \) are either both complex or both real, since they are the roots of a quadratic equation (see above). Given that \( W \) is an irreducible positive square matrix (as long as \( q \neq 1 \)), the Perron-Frobenius theorem holds, and thus \( W \) has a real eigenvalue. Thus, \( \lambda_{1, 2} \) are both real, and one of them is larger than the other, suppose \( \lambda_1 \). According to the theorem, there is a positive eigenvector \( v \) that corresponds to the largest eigenvalue and \emph{all} other positive eigenvectors of \( W \) are multiples of \( v \). Since the non-trivial equilibrium is a strictly positive vector, it must correspond to the largest eigenvalue, thus \( \lambda_1 \) where:
\[
