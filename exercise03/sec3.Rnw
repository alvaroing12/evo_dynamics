...
\begin{align*}
    \Pr[X \geq 1]
        ...
        &= 1 - \Pr[X = 0]
\end{align*}
where \( P[X = 0] \) is the probability of acquiring \emph{no} mutations at all. The probability that a single site does not acquire a mutation is, by definition, \( (1 - \mu) \). Mutations are supposed to occur independently at each site, therefore, the probability of no sites acquiring mutation is
\[
    P[X = 0] = (1 - \mu)^{10^6}
\]
Therefore,
